...
function visualizeAgeGroupByRegion() {
  ...
  const flatData = regions.flatMap(region => groupedData.get(region).entries().map(([ageGroup, count]) => ({ region, ageGroup, count })).toArray())
  console.log(flatData)

  const indexedData = d3.index(flatData, d => d.region, d => d.ageGroup)
  ...
  console.log(series(groupedData)[0])

  const regionSizes = regions.map(region => d3.sum(groupedData.get(region).values()))

  console.log(regionSizes)

  console.log(d3.max(regionSizes))
 ...
 visibleArea.append("g")
  ...
  const ageGroupVis = visibleArea.selectAll("g.ageGroupRects")
    .data(series)
    .join("g")
  ...
  ageGroupVis.selectAll("rect")
    ...
    .attr("width", d => regionScale(d.data[0]).bandwidth)
    .attr("y", d => sizeScale(d[0]))
    .attr("height", d => sizeScale(d[1]) - sizeScale(d[0]))
}
